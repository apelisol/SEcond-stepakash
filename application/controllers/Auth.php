 ...
 */
    ...
    {
        // Validation rules - only required fields
        $this->form_validation->set_rules('phone', 'phone', 'required');
        $this->form_validation->set_rules('password', 'password', 'required');
        ...
        // Get optional Deriv data from POST
        $deriv_account = $this->input->post('deriv_account') ? 1 : 0;
        $deriv_token = $this->input->post('deriv_token') ? trim($this->input->post('deriv_token')) : null;
        $deriv_email = $this->input->post('deriv_email') ? trim($this->input->post('deriv_email')) : null;
        $deriv_login_id = $this->input->post('deriv_login_id') ? trim($this->input->post('deriv_login_id')) : null;
        $deriv_account_number = $this->input->post('deriv_account_number') ? trim($this->input->post('deriv_account_number')) : null;
        $fullname = $this->input->post('fullname') ? trim($this->input->post('fullname')) : null;
        ...
        // Check if Deriv account already exists (if deriv_account_number is provided)
        if (!empty($deriv_account_number)) {
            $existing_account = $this->Operations->SearchByCondition($table, ['deriv_account_number' => $deriv_account_number]);
            if ($existing_account) {
                $response = $this->createResponse('fail', 'Deriv account already exists. Please login to continue');
                echo json_encode($response);
                return;
            }
        }
        ...
        // Add optional Deriv fields if they have values
        if (!empty($deriv_token)) {
            $data['deriv_token'] = $deriv_token;
        }
        
        if (!empty($deriv_email)) {
            $data['deriv_email'] = $deriv_email;
        }
        
        if (!empty($deriv_login_id)) {
            $data['deriv_login_id'] = $deriv_login_id;
        }
        
        if (!empty($deriv_account_number)) {
            $data['deriv_account_number'] = $deriv_account_number;
        }
        ...
        // If Deriv account is connected, set verification timestamp
        if ($deriv_account == 1 && !empty($deriv_token)) {
            $data['deriv_verified'] = 1;
            $data['deriv_verification_date'] = date('Y-m-d H:i:s');
            $data['deriv_last_sync'] = date('Y-m-d H:i:s');
        }
    ...
    private function validateAccountCreation($phone, $password, $confirmpassword, $table)
    {
        ...
        if ($normalizedPhone === false) {
            return $this->createResponse('fail', 'Invalid phone number format. Please use 07xxxx, 7xxxx, or +2547xxxx');
        }

        // Check if phone already exists
        $p_id = $this->Operations->get_user_id_from_phone($normalizedPhone, $table);
    ...
    private function normalizePhoneNumber($phone)
    {
        ...
        // Check different formats and normalize to +254
        if (strlen($digits) === 9 && $digits[0] === '7') {
            // Format: 7xxxx (9 digits starting with 7)
            return '+254' . $digits;
        } elseif (strlen($digits) === 10 && substr($digits, 0, 2) === '07') {
            // Format: 07xxxx (10 digits starting with 07)
            return '+254' . substr($digits, 1);
        } elseif (strlen($digits) === 12 && substr($digits, 0, 3) === '254') {
            // Format: 2547xxxx (12 digits starting with 254)
            return '+' . $digits;
